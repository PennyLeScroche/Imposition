...
class Roll:
    ...
    def __init__(self,pool_dice,opposition_dice):
        ...
        for x in range(len(self.opposition)):
            for y in range(len(self.pool)):
                if self.opposition[x] > self.pool[y] and self.pool[y] > 0:
                    print("removing",self.opposition[x],"from the opposition pool and",self.pool[y],"from the positive pool")
                    self.opposition[x] = 0
                    self.pool[y] = 0
        if len(self.pool)>0:
            ...
            if self.pool[0]<5:
                self.result = "failure"
            elif self.pool[0] > 4 and self.pool[0] < 10:
                self.result = "success"
            elif self.pool[0] > 9:
                self.result = "success+"
            else:
...
while(loop):
    ...
    for x in range(len(rolls)):
        if rolls[x].result == "success+":
            ...
            Failures+=1
        print(x+1,"|",pool_input,"|",opposition_input,"|",str(rolls[x].pool_results),"|",str(rolls[x].opposition_results),"|",rolls[x].pool,"|",rolls[x].result)
